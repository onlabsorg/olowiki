...
module.exports = function Server (options={}) {    
    ...
    server.get(`/:store/*.html`, (req, res, next) => {
        if (req.path.slice(-1) === "/") {
    ...
    // Serve the /bin store
    server.get('/bin/*.js', (req, res, next) => {
        const scriptPath = Path.join(publicPath, req.path);
        const script = fs.readFile(scriptPath, 'utf8', (err, data) => {
            if (err) {
    ...
    // Serve the /man store
    const manBackend = new FSBackend(Path.join(__dirname, '../public/man'));
    server.use('/man', new HTTPBackendServer(manBackend, auth.authorization.static));
