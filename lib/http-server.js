...
module.exports = (options={}) => async function (port) {
    ...
    options.allow = async req => {
        ...
        if (req.method === "GET") {
            let allowed = await document.expression.apply(acl.allow, "read", req.path, req.user.id);
            return Boolean(allowed);
        }
        
        if (req.method === "PUT" || req.method === "DELETE") {
            let allowed = await document.expression.apply(acl.allow, "write", req.path, req.user.id);
            return Boolean(allowed);
        }
        
        if (req.method === "POST") {
            let allowed = await document.expression.apply(acl.allow, "append", req.path, req.user.id);
            return Boolean(allowed);
        }
