...
function OloWebUI (options) {
    ...
    const oloWebUIComponent = {
        ...
        watch: {
            ...
            "editMode": function () {
                if (this.editMode) {
                    this.source = doc.dump( this.dotPath );
                }
                else {
                    doc.load( this.source, this.dotPath );
                    this.render();
                }
            },
            
            "dotPath": function (newDocSubPath, oldDocSubPath) {
                if (this.editMode) {
                    doc.load( this.source, oldDocSubPath );
                    this.source = doc.dump( newDocSubPath );
                } else {
                    this.render();
                }
            }            
        ...
        methods: {
            ...
            async render () {
                ...
                try {
                    this.value = await doc.evaluate( this.dotPath || "index" );
                }          
                catch (error) {
                    this.value = error;
                }
