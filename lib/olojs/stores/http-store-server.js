    ...
    router.put(`${storePath}/:subPath(*)`, (req, res, next) => {         
        ...
        .then((doc) => {
            if (doc === null && !auth.create(req.params.subPath, req.oloUser)) {
                res.status(403).send("Create permission denied.");
            }
            else if (doc !== null && !auth.write(doc, req.oloUser)) {
                res.status(403).send("Write access denied.");
            } 
            else {
                let newDoc = backend.createDocument(req.params.subPath, req.body);
                newDoc.set('author', req.oloUser.id);

                backend.writeDocument(req.params.subPath, newDoc)
                .then(() => {
                    res.status(200).send();
                })
                .catch(error => {
