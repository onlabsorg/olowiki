...
async function expeval (source, globals) {
    var expression = new Expression(source);
    var context = Expression.createContext(globals);
...
describe("Expression", () => {
    ...
    describe("A.B", () => {
        ...
        it("should return null if B starts with '__' and ends with '__'", async () => {
            var scope = {a:{_b:1, c_:2, _d_:3, _e__:4, __f_:5, __g__:6}};
            expect( await expeval("a._b", scope) ).to.equal(1);            
            expect( await expeval("a.c_", scope) ).to.equal(2);            
            expect( await expeval("a._d_", scope) ).to.equal(3);            
            expect( await expeval("a._e__", scope) ).to.equal(4);            
            expect( await expeval("a.__f_", scope) ).to.equal(5);            
            expect( await expeval("a.__g__", scope) ).to.equal(null);            
        }); 
    ...
    describe("A[B]", () => {
        ...
        describe.skip("when A is an object", () => {
    ...
    describe("name = expression", () => {
        ...
        it("should define a new name in the current namespace and assign it the result of the espression", async () => {
            var expression = new Expression("x = 10");
    ...
    describe("name: (params) -> expression", () => {
        ...
        it("should define a new function in the current namespace", async () => {
            var expression = new Expression("foo: (a,b) -> a+b");
            var context = Expression.createContext({
                $add: (x,y) => x+y                
            });
            await expression.evaluate(context);
            expect(context.foo).to.be.a("function");
            expect(await context.foo(1,2)).to.equal(3);
        }); 
