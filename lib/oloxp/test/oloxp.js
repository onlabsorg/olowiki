...
const oloxp = require("../src/oloxp/index");
...
suite("oloxp", () => {
    ...
    suite("oloxp.evaluate", () => {
        ...
        test("function call", (done) => {
            const context = {
                map: {
                    syncfn: function (caller, a,b) {
                        return {context:this, caller:caller, a:a, b:b}
                    },                
                    asyncfn: async function (caller, a,b) {
                        return {context:this, caller:caller, a:a, b:b}
                    },                
                    callable: {
                        call: async function (self, caller, a,b) {
                            return {context:self, caller:caller, a:a, b:b}
                        }
                    },
                    nofunc: 10
                }
            ...
            async function runtest () {
                ...
                expect(val1).to.deep.equal({context:context, caller:context.map, a:1, b:2});
                expect(val2).to.deep.equal({context:context, caller:context.map, a:3, b:4});
                expect(val3).to.deep.equal({context:context, caller:context.map, a:5, b:6});
